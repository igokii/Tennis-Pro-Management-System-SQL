...
BEGIN
    ...
    VALUES (8, 1, 1, 1, 'Test Tournament', '2024-12-01', 'Final', 120, 4);
...
BEGIN
    ...
    INSERT INTO matches (referee_id, player1_id, player2_id, winner_id, tournament, match_date, round, duration, court_id) VALUES
        (8, 1, 3, 1, 'Test T1', '2025-01-15', 'R1', 120,4),
        (8, 2, 4, 2, 'Test T2', '2025-01-15', 'R2', 130,4),
        (8, 5, 6, 5, 'Test T3', '2025-01-15', 'R3', 140,4);
    -- Intentar el 4º
    INSERT INTO matches (referee_id, player1_id, player2_id, winner_id, tournament, match_date, round, duration, court_id)
    VALUES (8, 1, 2, 1, 'Test T4', '2025-01-15', 'R4', 110,4);
...
BEGIN
    ...
    INSERT INTO matches (referee_id, player1_id, player2_id, winner_id, tournament, match_date, round, duration, court_id)
    VALUES (7, 2, 3, 2, 'Test Tournament', '2025-02-01', 'Final', 150,4);
...
-- RA-03a,b,c,d: Wimbledon -> Hierba, Roland Garros -> Arcilla, US Open/Australian Open ->Dura

DELIMITER //
CREATE OR REPLACE PROCEDURE p_test_ra03a_surface_error() 
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
        CALL p_log_test('RA-03a', 'RA-03a:  Un partido debe tener la superficie de pista correcta en función del torneo.', 'PASS');
	
    CALL p_populate_db();
    
    -- Wimbledon
    INSERT INTO matches (referee_id, player1_id, player2_id, winner_id, tournament, match_date, round, duration, court_id)
    VALUES (8, 1, 2, 1, 'prueba Wimbledon 2025', '2025-01-1', 'R4', 110,2);
    CALL p_log_test('RA-03a', 'ERROR: Se permitió una pista incorrecta para un torneo Wimbledon', 'FAIL');
END //
DELIMITER ;

-- según el enunciado del examen, basta con probar solo wimbledon. Yo he decidido probar también Roland garros, US Open y Australian Open.

DELIMITER //
CREATE OR REPLACE PROCEDURE p_test_ra03b_surface_error() 
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
        CALL p_log_test('RA-03b', 'RA-03b:  Un partido debe tener la superficie de pista correcta en función del torneo.', 'PASS');
	
    CALL p_populate_db();
    -- Roland Garros
    INSERT INTO matches (referee_id, player1_id, player2_id, winner_id, tournament, match_date, round, duration, court_id)
    VALUES (8, 1, 2, 1, 'prueba Roland Garros 2025', '2025-01-1', 'R4', 110,1);
    CALL p_log_test('RA-03b', 'ERROR: Se permitió una pista incorrecta para un torneo Roland Garros', 'FAIL');
END //
DELIMITER ;

DELIMITER //
CREATE OR REPLACE PROCEDURE p_test_ra03c_surface_error()
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
        CALL p_log_test('RA-03c', 'RA-03c:  Un partido debe tener la superficie de pista correcta en función del torneo.', 'PASS');
	
    CALL p_populate_db();
    
    -- US Open
    INSERT INTO matches (referee_id, player1_id, player2_id, winner_id, tournament, match_date, round, duration, court_id)
    VALUES (8, 1, 2, 1, 'prueba US Open 2025', '2025-01-1', 'R4', 110,1);
    CALL p_log_test('RA-03c', 'ERROR: Se permitió una pista incorrecta para un torneo US Open', 'FAIL');
END //
DELIMITER ;

DELIMITER //
CREATE OR REPLACE PROCEDURE p_test_ra03d_surface_error() 
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
        CALL p_log_test('RA-03d', 'RA-03d:  Un partido debe tener la superficie de pista correcta en función del torneo.', 'PASS');
	
    CALL p_populate_db();
    
    -- Australia Opens
    INSERT INTO matches (referee_id, player1_id, player2_id, winner_id, tournament, match_date, round, duration, court_id)
    VALUES (8, 1, 2, 1, 'prueba Australian Open 2025', '2025-01-1', 'R4', 110,1);
    CALL p_log_test('RA-03d', 'ERROR: Se permitió una pista incorrecta para un torneo Australia Open', 'FAIL');
END //
DELIMITER ;
-- =============================================================
...
BEGIN
    ...
    CALL p_test_ra03a_surface_error();
    CALL p_test_ra03b_surface_error();
    CALL p_test_ra03c_surface_error();
    CALL p_test_ra03d_surface_error();
