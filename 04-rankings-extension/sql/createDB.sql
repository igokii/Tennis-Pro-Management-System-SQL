...
CREATE TABLE rankings (
   ...
   CONSTRAINT rn_04_rankingPosition CHECK (position > 0 AND position <= 1000)
);
...
-- ============================================================================
-- RA-02: Posición del ranking no puede cambiar más de 5o puestos entre dos fechas consecutivas
-- ============================================================================

-- Validar que el ganador del set sea uno de los jugadores del partido
-- y que no se excedan los 5 sets por partido
CREATE OR REPLACE TRIGGER t_biu_rankings_ra02
BEFORE INSERT OR UPDATE ON rankings
FOR EACH ROW
BEGIN
    DECLARE difference1 INTEGER;
    DECLARE difference2 INTEGER;
    DECLARE previous_position INTEGER;
    DECLARE future_position INTEGER;
    
    SELECT r.position INTO previous_position FROM rankings r 
	 ...
	 -- por si inserto una posición previa a la que ya estaba insertada			 
     SELECT r.position INTO future_position FROM rankings r 
   				WHERE r.person_id = new.person_id
					 	 AND r.date > new.date
    					 AND ranking_id <> IFNULL(new.ranking_id, 0)
					ORDER BY r.date 
    ...
    SET difference1 = ABS(new.position - IFNULL(previous_position, new.position));
    SET difference2 = ABS(new.position - IFNULL(future_position, new.position));
    
    IF(difference1 > 50 OR difference2 > 50) THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'RA-02: La posición del ranking no puede cambiar más de 50 puestos entre dos fechas consecutivas';
    END IF;
END //
