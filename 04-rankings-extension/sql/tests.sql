...
-- RA-02: no cambiar 50 puntos de ranking entre dos fechas consecutivas
DELIMITER //
CREATE OR REPLACE PROCEDURE p_test_rankings_ra02()
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
        CALL p_log_test('RA-02', 'RA-02: No se permiten cambios de más de 50 puntos', 'PASS');

    CALL p_populate_db();
    
    -- insertamos puesto correcto. Sabemos que el puesto posterior es 34.
    	INSERT INTO rankings (person_id, date, position) VALUES
		(2, DATE('2021-01-01'), 84);
		SET @ultimo_id = LAST_INSERT_ID();
		-- cambiamos la fecha para que quede tras el 34. esto supone una diferencia 
		-- de mas de 50 puntos con la siguiente fecha.
		UPDATE rankings 
			SET date = '2023-01-01' 
		WHERE ranking_id = @ultimo_id;

		CALL p_log_test('RA-02', 'ERROR: Se permitió un cambio superior a 50 ptos', 'FAIL');
END //
DELIMITER ;
