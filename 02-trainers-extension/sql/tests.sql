...
DELIMITER //
CREATE OR REPLACE PROCEDURE p_test_trainers_max_2_players()
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
        CALL p_log_test('RA-02', 'RA-02: Un entrenador no puede entrenar a más de 2 tenistas simultáneamente', 'PASS');

    CALL p_populate_db();
    -- creo el entrenador
    INSERT INTO people (person_id, name, age, nationality) VALUES (101, 'Test trainer', 50, 'Chile');
    INSERT INTO trainers (trainer_id, experience, specialty) VALUES
    		(101, 35, 'Dobles');
   -- creo los jugadores
   
    INSERT INTO people (person_id, name, age, nationality) VALUES 
	 (200, 'Test player 1', 20, 'Chile'), 
	 (201, 'Test player 2', 21, 'Chile'), 
	 (202, 'Test player 3', 22, 'Chile');
    INSERT INTO players (player_id, ranking, trainer_id) VALUES 
        (200, 100, 101),
        (201, 200, 101),
        (202, 300, 101);
    CALL p_log_test('SET-2', 'ERROR: Se permitió más de 5 sets en un partido', 'FAIL');
...
BEGIN
    ...
    CALL p_test_trainers_max_2_players();
